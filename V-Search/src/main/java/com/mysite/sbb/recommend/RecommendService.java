...
public class RecommendService {
	...
	@Transactional
	public boolean toggleRecommendation(int videoNo) {
		// 현재 로그인한 사용자 이름 가져오기
		...
		if (existingRecommendation == null) {
			...
			return existingRecommendation.isRecommendCheck(); // 토글 후의 추천 상태를 반환
		}
	}
	...
	public boolean hasUserRecommended(int videoNo) {
		...
		int userNo = userService.getUserNO(username);
		Recommend existingRecommendation = recommendRepository.findByVideoNoAndUserNo(videoNo, userNo);
		return existingRecommendation != null && existingRecommendation.isRecommendCheck();
	}
}
